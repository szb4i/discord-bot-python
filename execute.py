...
class Execute():
    ...
    def open_long(self, symbol):
        ...
        order = self.binance_client.futures_create_order(symbol=symbol, side=SIDE_BUY, type=FUTURE_ORDER_TYPE_MARKET, quantity=TRADE_QUANTITY, isIsolated=True)
        print('action: long opened')
        print(order)

    def close_long(self, symbol):
        self.binance_client.futures_change_leverage(symbol=symbol, leverage=LEVERAGE)
        order = self.binance_client.futures_create_order(symbol=symbol, side=SIDE_SELL, type=FUTURE_ORDER_TYPE_MARKET, quantity=TRADE_QUANTITY, isIsolated=True)
